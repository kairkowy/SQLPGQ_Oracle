...
이 쿼리 예제는 Oracle Property Graph Release 24.4 with oracle 23.8을 기준으로 작성됐습니다. 보다 상세한 내용은 오라클 Graph Developer's Guide for Property Graph 문서를 참고하세요.
...
SELECT * FROM GRAPH_TABLE (students_graph
  ...
  (a IS person) -[e IS friends] - (b IS person WHERE b.name='Alice' OR b.name='Mary')
  WHERE (a.name='Alice' OR a.name='Mary')
  COLUMNS (vertex_id(a) as person_a, edge_id(e) as 만난날, vertex_id(b) as person_b)
);
```
